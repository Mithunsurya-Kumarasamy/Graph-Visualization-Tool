...
def calculate_prims_mst():
    """Calculates MST using a manual Prim's implementation and updates visualization variables."""
    ...
    if not nx.is_connected(graph):
        ...
        try:
            ...
            subgraph_to_process = graph.subgraph(largest_cc_nodes).copy() # Use a copy
        except ValueError:
            print("No components to run on.")
            return
    else:
        # If already connected, just use the graph itself (or a copy if you plan to modify)
        # Reading is safe, so this is fine.
    ...
    if subgraph_to_process.number_of_edges() == 0 and subgraph_to_process.number_of_nodes() > 0:
        ...
        # No edges to add to mst_edges (it's already cleared)
        return

    # --- Manual Prim's Algorithm Implementation ---
    try:
        # mst_edges is already cleared by clear_visualizations()
        prims_weight = 0
        
        # A set to keep track of vertices already included in the MST
        in_mst = set()
        
        # A min-priority queue (heap) to store edges (weight, source_node, dest_node)
        # We'll use this to find the cheapest edge crossing the "frontier"
        edge_heap = []

        # 1. Pick a starting node
        # We know the subgraph has at least one node from the checks above
        ...
        for neighbor, data in subgraph_to_process.adj[start_node].items():
            ...
            # The pre-check should ensure weight is not None, but we double-check
            if weight is not None:
                heapq.heappush(edge_heap, (weight, start_node, neighbor))

        # 3. Loop until all nodes are in the MST or the heap is empty
        num_nodes_in_component = subgraph_to_process.number_of_nodes()
        while edge_heap and len(in_mst) < num_nodes_in_component:
            
            # 4. Get the cheapest edge from the heap
            weight, u, v = heapq.heappop(edge_heap)

            # 5. Check if the destination node 'v' is already in the MST
            # If it is, this edge would create a cycle, so we skip it.
            if v in in_mst:
                continue

            # 6. Add the new node 'v' to the MST
            in_mst.add(v)
            
            # Add the edge to our results
            mst_edges.add(tuple(sorted((u, v))))
            prims_weight += weight

            # 7. Add all *new* adjacent edges from 'v' to the heap
            for neighbor, data in subgraph_to_process.adj[v].items():
                # Only add edges leading to nodes *not* already in the MST
